        ...
        <div class="col-md-12">
            <div class="col-md-12">
                <div class="card">
                    ...
                    {{-- <hr /> --}}
                    <form action="{{ route('basicplanmake.update', $planmakes->id) }}" method="POST" id="makeplan" enctype="multipart/form-data" class="form theme-form dark-inputs">
                        @csrf
                        @method('PATCH')
                        <div class="card-body">
                            <div class="row">
                                            ...
                                            <table class="w-100" id="dynamicTable"  style="border: 1px solid #eee;" name="myForm">
                                                ...
                                                <tbody id="row_body1">
                                                    ...
                                                    @foreach ($planmakes_item as $plan_item)
                                                        ...
                                                        @endif
                                                        <tr id="rowA{{ $i }}">
                                                            <td><input type="hidden" class="w-100" name="basic[{{$i}}][id]" id="basic_id{{$i}}" data-srno="{{$i}}" value="{{$plan_item->id}}" readonly><input type="text" class="form-control" name="basic[{{ $i }}][items]" id="basic{{ $i }}" value="{{ $plan_item->items }}"></td>
    ...
    <script type="text/javascript">
        $().ready(function() {
            ...
            $('#add_rowA').on('click', function() {
                ...
                row_countA++;
                var content =
                '<tr id="rowA'+row_countA+'"><td><input type="text" class="form-control" name="basic['+row_countA+'][items]" id="basic'+row_countA+'"></td><td><a href="javascript:void(0);" class="remove_plan text-danger" data-row="'+row_countA+'"><i class="fa fa-trash"></i></a></td></tr>';
    ...
    <script>
        ...
        $(document).on('click', '.remove_plan', function() {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url:'{{ route("basicplanmakeitem/delete") }}',
                        method:'POST',
                        dataType:"json",
                        data: {
                            "_token": "{{ csrf_token() }}",
