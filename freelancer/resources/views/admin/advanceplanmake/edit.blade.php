            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h4>Edit advance Plan</h4>
                    </div>
                    {{-- <div class="text-start">
                        <a href="{{ route('subcategory.index') }}" class="btn btn-primary btn-sm ms-4" id="addRow">Back</a>
                    </div> --}}
                    {{-- <hr /> --}}
                    <form action="{{ route('advanceplanmake/update') }}" method="POST" id="makeplan" enctype="multipart/form-data" class="form theme-form dark-inputs">
                        @csrf
                        @method('PATCH')
                        <div class="card-body">
                            <div class="row">
                                ...
                                <div class="col-lg-6 col-md-6">
                                    <div class="pricingtable p-2">
                                        <div><h3>Advance Details</h3></div><br>
                                            ...
                                            <table class="w-100" id="dynamicTable"  style="border: 1px solid #eee;" name="myForm">
                                                ...
                                                <tbody id="row_body1">
                                                    ...
                                                    @foreach ($planmakes_item as $plan_item)
                                                        ...
                                                        @endif
                                                        <tr id="rowA{{ $i }}">
                                                            <td><input type="hidden" class="w-100" name="advance[{{$i}}][id]" id="advance_id{{$i}}" data-srno="{{$i}}" value="{{$plan_item->id}}" readonly><input type="text" class="form-control" name="advance[{{ $i }}][items]" id="advance{{ $i }}" value="{{ $plan_item->items }}"></td>
                                ...
                                <input type="hidden" name="plan_name" value="advance" readonly>
                                <div class="col-lg-6 col-md-6">
                                    <div class="pricingtable p-2">
                                        <div><h3>Advance plan add Technology</h3></div><br>
    ...
    <script type="text/javascript">
        $().ready(function() {
            ...
            $('#add_rowA').on('click', function() {
                ...
                row_countA++;
                var content =
                '<tr id="rowA'+row_countA+'"><td><input type="text" class="form-control" name="advance['+row_countA+'][items]" id="advance'+row_countA+'"></td><td><a href="javascript:void(0);" class="remove_plan text-danger" data-row="'+row_countA+'"><i class="fa fa-trash"></i></a></td></tr>';
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    if (data.status == 0) {
                        ...
                        window.location.href = '{{route("advanceplanmake/index")}}';
                    }
                }
            })
        })
    ...
    <script>
        ...
        $(document).on('click', '.remove_plan', function() {
            ...
            .then((willDelete) => {
                if (willDelete) {
                    $.ajax({
                        url:'{{ route("advanceplanmakeitem/delete") }}',
                        method:'POST',
                        dataType:"json",
                        data: {
                            "_token": "{{ csrf_token() }}",
